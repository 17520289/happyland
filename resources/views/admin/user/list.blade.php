            ...
            <table id="crudTable" class="bg-white table table-striped table-hover nowrap rounded shadow-xs border-xs mt-2"
                ...
                <thead>
                    <tr>
                        ...
                        data-force-export => force export even if field are hidden --}} {{-- If it is an export field only, we are done. --}}
...
@section('after_scripts')
    ...
    <script type="text/javascript">
        function actionDelete(cb ,user_id) {
            console.log(cb.checked);
            url = "{{ route('user.update-status.post') }}" + "?user_id="+ user_id +"&&action_active="+ cb.checked;
            $.ajax({
                type: 'POST',
                url: url,
                data: {
